<template>
        ...
        <div class="product-details mr-2">
          <div class="d-flex flex-row align-items-center">
            <button class="btn btn-primary btn-block d-flex justify-content-between mt-3" type="button">
              <router-link to="/" style="text-decoration: none; color: white"><i class="fa fa-long-arrow-left"
                  style="margin: 5px 10px 0px 0px"></i><span class="ml-2">Continue Shopping</span>
              </router-link>
            </button>
          </div>
          <hr />
          ...
          <div class="d-flex justify-content-between">
            <span>You have {{ Object.keys($store.state.Order).length }} items in
              your cart</span>
          </div>

          <div class="d-flex justify-content-between align-items-center mt-3 p-2 items rounded"
            v-for="(product, index) in $store.state.Order" :key="product.id">
            <div class="d-flex flex-row">
              <img class="rounded" :src="product.img" width="40" />
              <div class="ml-2">
                <span class="font-weight-bold d-block">{{ product.name }}&nbsp;</span>
              </div>
            </div>
            <div class="d-flex flex-row align-items-center">
              <span class="d-block" style="margin: 0px 257px 0px 0px"><button @click="
                                  if ($store.state.Order[index]['count'] > 0) {
                $store.state.Order[index]['count']--;
              }
                total_price();
                " class="btn btn-primary" type="button" style="margin: 0px 10px 0px 10px; padding: 0px 7px 0px 6px">
  ...
  $store.state.Order[index]['count']++;
  total_price();
  " class="btn btn-primary" type="button" style="margin: 0px 0px 0px 10px; padding: 0px 5px 0px 5px">
                  <span class="ml-2">+</span>
                </button></span><span class="d-block ml-5 font-weight-bold">${{ product.Price }}</span><button
                @click="deleteproduct(index)" class="btn btn-primary" type="button" style="
      ...
      <div class="col-md-4">
        <div class="payment-info">
          ...
          <span class="type d-block mt-3 mb-1">Card type</span><label class="radio">
          ...
          <div>
            <label class="credit-card-label">Name on card</label><input type="text" class="form-control credit-inputs"
              placeholder="Name" />
          </div>
          <div>
            <label class="credit-card-label">Card number</label><input type="text" class="form-control credit-inputs"
          ...
          <div class="d-flex justify-content-between information">
            <span>Total(Incl. taxes)</span><span>${{ totalprice + 20 }}</span>
          </div>
          <button @click="checkout()" class="btn btn-primary btn-block d-flex justify-content-between mt-3" type="button">
            <span>${{ totalprice + 20 }}</span><span>Checkout<i class="fa fa-long-arrow-right ml-1"></i></span>
          </button>
        </div>
      </div>
    </div>
